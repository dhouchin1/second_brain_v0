...
"""LEGACY BACKUP — Snapshot prior to capture enhancement.

Kept for reference only. Do not import or extend. See `app.py` for the active
application and `services/` for the unified service layer.
"""
...
async def capture(
    ...
    if file:
        ...
        safe_name = f"{timestamp}-{(file.filename or 'audio').replace(' ', '_')}"
        audio_path = settings.audio_dir / safe_name
        # Stream to disk to avoid large memory spikes/timeouts
        max_bytes = 200 * 1024 * 1024  # 200 MB safety limit
        written = 0
        with open(audio_path, "wb") as out_f:
            while True:
                ...
                if written > max_bytes:
                    ...
                    if "application/json" in request.headers.get("accept", ""):
                        raise HTTPException(status_code=413, detail="File too large (limit ~200MB)")
                    return RedirectResponse("/", status_code=302)
                out_f.write(chunk)
    ...
    if file:  # Audio file - requires processing
        ...
        # Processing handled by global FIFO worker; no per-request background task
...
async def queue_status(current_user: User = Depends(get_current_user)):
    ...
    conn = get_conn()
    c = conn.cursor()
    pending = c.execute("SELECT COUNT(*) FROM notes WHERE user_id=? AND status='pending'", (current_user.id,)).fetchone()[0]
    processing = c.execute("SELECT COUNT(*) FROM notes WHERE user_id=? AND status='processing'", (current_user.id,)).fetchone()[0]
    completed = c.execute("SELECT COUNT(*) FROM notes WHERE user_id=? AND status='complete'", (current_user.id,)).fetchone()[0]
    conn.close()
    return {"pending": pending, "processing": processing, "complete": completed}
